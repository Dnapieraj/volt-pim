// Runs on every production boot (see package.json "start"), but only
// populates demo data the first time — if the database already has
// users, it exits immediately so real production data is never touched
// or wiped on redeploy/restart. Plain CommonJS + only runtime deps
// (@prisma/client, bcryptjs) so it needs no dev tooling (tsx/ts-node)
// in the production container.
const { PrismaClient, ProductStatus, UserRole } = require("@prisma/client");
...
const demoUsers = [
  { email: "admin@voltpim.dev", name: "Daniel", role: UserRole.ADMIN },
  { email: "edytor@voltpim.dev", name: "Anna", role: UserRole.EDITOR },
  { email: "podglad@voltpim.dev", name: "Marek", role: UserRole.VIEWER },
];

async function main() {
  const userCount = await prisma.user.count();
  if (userCount > 0) {
    console.log("Seed pominięty: baza już ma dane (znaleziono użytkowników).");
    return;
  }

  console.log("Baza pusta — wgrywam dane demo...");
...
main()
  ...
  .catch(async (error) => {
    console.error("Seed nieudany (aplikacja i tak wystartuje):", error);
    await prisma.$disconnect();
  });
